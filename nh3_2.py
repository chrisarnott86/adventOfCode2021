...
#def c02_checks(this_list, pos_var, bit_list, position):
...
def c02_checks(this_list, pos_var, bit_list, position):
    # Initially this code runs to check C02 Binary Numbers.
    #print("Values Kept for Pos1:", position_check(this_list, pos_var, bit_list, position))
    position_check_2(this_list, pos_var, bit_list, position)
    if len(pos_var) == 1:
        global c02_scrub_rating_final
        c02_scrub_rating_final = result_print(pos_var)
        print("C02 scrubbing Rating Is:", c02_scrub_rating_final)
